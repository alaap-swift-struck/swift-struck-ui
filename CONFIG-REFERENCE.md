...
**`Field` props (not config): `shape`.** The required-ring is drawn around whatever the Field wraps, so it has to match that thing's shape:

| `shape`           | Ring        | Use for                                                                               |
| ----------------- | ----------- | ------------------------------------------------------------------------------------- |
| `input` (default) | yes, rect   | One rectangular control — an Input, Textarea, Select/dropdown trigger. **Unchanged.** |
| `pill`            | yes, pill   | A rounded/pill control; the ring follows its corners instead of boxing it.            |
| `group`           | **no ring** | Several gap-separated controls (pills, chips, a radio row).                           |

`group` suppresses the ring because a single rectangle would enclose the **gaps between** the controls — a gold box around a group, not around a field. The label's `*` carries "required" there instead. Under the hood the ring radius is the CSS var `--ss-ring-radius` (defaults to the input radius), so a custom control can publish its own corners. `ringed={false}` still disables the ring entirely for composite controls that draw their own.
...
**Primitives:** the search box is the **`SearchInput`** primitive (Input + lucide Search + a clear ✕, debounced via `debounceMs`); the facet row is the **`FilterBar`** primitive (Select, searchable combobox, chips, or a numeric range + "Clear all", keyboard-operable, polite live count).

**Clearing a filter — two levels.** Every ACTIVE facet carries its own **✕ beside its control**, so a user can drop one filter without rebuilding the rest; **"Clear all"** (shown whenever anything is active, including the search box) resets everything. Both emit through the same `onChange(field, "")` / `onClearAll()` seam, and `CollectionFrame` deletes the key on an empty value — so clearing is a complete one-action reset with no host code. Each ✕ is a real `<button>` with an accessible name (`Clear <label>`) and a focus stop, **not** an icon inside the trigger: an svg nested in a `Button` is removed from hit-testing by the button's `[&_svg]:pointer-events-none`, which silently makes it unclickable and keyboard-unreachable. Chips are their own affordance — clicking the active chip toggles it off. Both debounce through the shared **`useDebouncedCallback`** hook (the `use-debounce` primitive) — one implementation, no repeat. `List`/`CardGrid` get all of this by rendering inside `CollectionFrame` (the gallery shows the pattern).
...
Choice's closed trigger truncates with a `title` fallback, and a host can sidestep truncation entirely with `ChoiceOption.triggerLabel` (see Choice). Filter-bar triggers carry a `max-w` so their `truncate` actually engages — a `truncate` with no width ceiling never fires.
...
**`ChoiceOption`** — `{ value, label, triggerLabel? }`. `triggerLabel` is a shorter label used **only in the closed trigger**; the menu always shows the full `label`. Use it when the full name is far wider than the control (`"INR"` in the control, `"INR — Indian Rupee"` in the list) so the trigger never has to truncate at all. Falls back to `label`.

**Inside a Dialog — pass `modal`.** A dropdown is portaled to `<body>`, so one opened inside a Dialog/Sheet lands outside the dialog's DOM subtree. The dialog's scroll lock then preventDefaults wheel/touchmove over it: **typing filters fine, but the list won't scroll.** Set `modal` and the popover takes its own scroll lock:

```tsx
<Choice modal options={currencies} value={v} onChange={setV} config={cfg} />
```

It is **off by default on purpose** — a modal popover also traps focus and blocks outside clicks, so in a normal page you could no longer click straight from one control to the next. The same prop exists on **`FilterBar`**, **`SortControl`** and **`CollectionFrame`** (which forwards it to all three), so a collection rendered in a dialog stays usable.
...
Each `StatItem` is `{ id, label, value, delta, trend }` plus an optional
`icon` — a decorative glyph drawn top-right of the card, beside the label. The
label truncates and the glyph never shrinks, so a long label can't push it out.
Omit it and the card is exactly as it was.
...
**`RecordDetailConfig` (Record Detail)** — the scaffold: a header (`title`, `subtitle`, `avatarSrc`/`avatarFallback`/`avatarShape`, `actions` slot — all props) above its `children` (typically a `TabsView` composing the two blocks above). Pass neither `avatarSrc` nor `avatarFallback` and the header draws no avatar at all; `avatarShape="square"` crops a logo or wordmark as a rounded square instead of a disc.
...
`TabItem`: `value` (id) · `label` (shown) · `icon` (a **string** is read as a lucide name, kebab-case e.g. `"inbox"`, and `""` = none, so a tab set stays serialisable data; any other node — a product logo, a type mark, a coloured dot — is rendered as-is) · `badge` (a count or tag e.g. `"24"`/`"New"`; `""` = none) · `badgeVariant` (`""` = neutral count chip, else a Badge variant like `"destructive"` to colour-code). A bar that overflows its container scrolls horizontally rather than spilling out.
...
| `header`       | `{ title; subtitle?; avatar?; avatarShape? }`                    | detail: which record columns feed the header. **Omit `avatar` and no avatar is drawn at all** — not an initials disc: a screen with no picture concept shouldn't open with two letters of its own title in a circle. `avatarShape` is `"circle"` (default) or `"square"` — square for a logo or wordmark, which a circular crop makes unreadable.                                                                      |
| `tabs`         | `RecipeTab[]`                                                    | detail: named tabs, each a block (`description` / `fields` / `activity` / `list`). `RecipeTab` = `{ key, label, icon?, badge?, badgeVariant?, block }` — `badge` is a **pre-formatted** count/status shown as a pill after the label (`"12"`, `"3 new"`); the renderer never computes it. `badgeVariant` is typed to the Badge variant union, so an unknown tone is a compile error rather than a wrong-looking pill.  |
...
| `leading`      | `string`                                                         | list (`display: "list"`/`"cards"`): the row column holding that record's picture — an avatar, logo or type mark the **host** has already rendered into the row (a row value is `unknown`, so a React node is a legal one). Drawn in the List's `leading` slot / the CardGrid's `media` slot, and read exactly as `fields[0]` is read for the title. Omit and nothing is drawn — there is deliberately no default mark. |
